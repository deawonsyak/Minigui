...
#include "mgdconfig.h"
#ifdef RCSID
static char *rcsid = "$Id: flash.cc,v 1.12 2006-04-25 07:26:36 jpzhang Exp $";
#endif
...
#ifdef _NOUNIX_
static void inline getvxworkstime(struct timeval * clocktime)
{
   struct timespec ts;
   clock_gettime(CLOCK_REALTIME,&ts);
   clocktime->tv_sec=ts.tv_sec;
   clocktime->tv_usec=ts.tv_nsec/1000;   
}
#endif
...
//printf("enter flashparse nn is %d, mm is %d*************\n",nn,mm);

	return status;
}

void
FlashGetInfo(FlashHandle flashHandle, FlashInfo *fi)
{
	FlashMovie *fh;
...
long FlashGraphicInit(FlashHandle flashHandle, FlashDisplay *fd,int sound)
{
	FlashMovie *fh;

	fh = (FlashMovie *)flashHandle;

	fh->gd = new GraphicDevice(fd);
          ...
          FlashEvent *fe, struct timeval *wakeDate)
{
	...
	switch (flag & FLASH_CMD_MASK) {
		...
		case FLASH_REWIND:
			...
			break;
		case FLASH_STEP:
			//hz fh->main->program->nextStepMovie();
        	wakeUp = fh->processMovie(fh->gd, fh->sm, false);
	...
	//	gettimeofday(wakeDate,0);
		...
		// Play frame
        wakeUp = fh->processMovie(fh->gd, fh->sm, true);//
	}
