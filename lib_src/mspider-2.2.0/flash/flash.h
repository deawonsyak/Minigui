...
#define FLASH_VERSION_STRING "Version 0.4.9"
...
typedef struct FlashDisplay {
    ...
    int width;
    int height;
    int depth;    /* only depth=16 is handled */
    int flash_refresh;
    /* Clipping region */
    int clip_x, clip_y;
...
extern FlashHandle FlashNew(void);
extern void FlashGetInfo(FlashHandle fh, FlashInfo *fi);
extern long FlashGraphicInit(FlashHandle fh, FlashDisplay *fd,int sound);
