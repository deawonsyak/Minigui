...
#ifdef RCSID
static char *rcsid = "$Id: program.cc,v 1.10 2006-04-25 07:26:36 jpzhang Exp $";
#endif
...
Program::~Program()
{
    ...
    if (frames != NULL) {
	    for(i=0;i<nbFrames;i++)
		{
			...
			if(frames[i].sndblock) free(frames[i].sndblock); 
...
void *streamplay(void *arg)
{
    ...
    //printf("the start frame is %d and total frame is %d##############\n",sp->startFrame,sp->totalFrames);
  ...
  //  printf("movie is %d\n",movie->sound);
...
#if 1
	if(frames[f].sndblock && (soundId == -1) && sm&&(soundsecond==1))
        {
            soundsecond=0;
			sp = new StreamPlay;
			sp->sm = sm;
	...
	if (status & GOTO) {
		if (nextFrame < nbFrames) {
			...
			if (nextFrame != f)
			if (movieStatus == MoviePaused) runFrame(gd,sm,nextFrame,1);
			update = 1;
		}
	}
