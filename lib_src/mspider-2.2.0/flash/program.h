...
struct Frame {
	...
	SoundStreamBlock * sndblock;  //hz add,   StreamBlock for this frame 
};
...
struct Program {
	...
	pthread_t    thread; //hz add for SoundStream
	...
	void	 continueMovie();
	void     nextStepMovie();
	void	 gotoFrame(GraphicDevice *gd, SoundMixer *, long f);
	...
	void	 advanceFrame();
    void     addSoundStreamInCurrentFrame(SoundStreamBlock * soundblock);  //hz add
	void	 addControlInCurrentFrame(Control *ctrl);
	void	 setGetUrlMethod( void (*)(char *, char *, void *), void *);
	void	 modifySettings(long flags);
	long	 searchFrame(GraphicDevice *gd, SoundMixer *, char *, char *);
