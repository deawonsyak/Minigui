...
#ifdef RCSID
static char *rcsid = "$Id: script.cc,v 1.5 2006-02-17 06:22:10 jpzhang Exp $";
#endif
...
//	printf("Code %d, tagLen %8u,tag time %d \n", code, m_tagLen,i++);
	//printf("%d\n", i++);
...
CInputScript::ParseData(FlashMovie *movie, char * data, long size)
{
    ...
    if (needHeader) {
	    ...
	    frameRate = GetWord() >> 8;
	    frameCount = GetWord();
//printf("frameRate %d,frameCount %d*************\n",frameRate,frameCount);
