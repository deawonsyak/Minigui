...
static CookieControlAction Cookies_control_check(const mSpiderUrl *url);
static CookieControlAction Cookies_control_check_domain(const char *domain);
static int Cookie_control_init(void);
static void Cookies_parse_ports(const mSpiderUrl *set_url, CookieData_t *cookie,
                                const char *port_str);
...
static FILE *Cookies_fopen(const char *filename, gchar *init_str)
{
   ...
   printf("##############Cookies_fopen filename:%s\n", filename);

	
   if ((F_in = fopen(filename, "r+")) == NULL) {
   	  printf("##############Cookies_fopen NULL\n");
      /* Create the file */
...
void a_Cookies_init()
{
   ...
   filename = a_Misc_prepend_user_home(".mspider/cookies");
   file_stream = Cookies_fopen(filename, NULL);
   g_free(filename);

   if (!file_stream) {
      DEBUG_MSG(10, "ERROR: Can't open ~/.mspider/cookies, disabling cookies\n");
      return;
   }
   ...
   if (disabled) {
      DEBUG_MSG(10, "The cookies file has a file lock:\n"
                    "  disabling cookies in this mspider!\n");
      fclose(file_stream);
      return;
   }
 ...
 * Parse one cookie...
 */
static CookieData_t *Cookies_parse_one(const mSpiderUrl *set_url,
                                       char **cookie_str)
{
   CookieData_t *cookie;
   char *str = *cookie_str;
   ...
   while (*str != '\0' && *str != ',') {
      ...
      if (num_attr == 0) {
         ...
         if (!max_age && !discard) {
            ...
            if (value) {
               cookie->expires_at = a_Cookies_create_timestamp(value);
               cookie->session_only = FALSE;
               g_free(value);
            } else {
               DEBUG_MSG(8, "Failed to parse cookie value!\n");
 ...
 */
static GSList *Cookies_parse_string(const mSpiderUrl *set_url,
                                    char *cookie_string)
{
   CookieData_t *cookie;
   GSList *ret = NULL;
 ...
 */
void a_Cookies_set(GList *cookie_strings, const mSpiderUrl *set_url)
{
   CookieControlAction action;
   GSList *list;
 ...
 */
static gboolean Cookies_match(CookieData_t *cookie, const mSpiderUrl *url,
...
static int Cookie_control_init(void)
{
   ...
   /* Get a file pointer */
   filename = a_Misc_prepend_user_home(".mspider/cookiesrc");
