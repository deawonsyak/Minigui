...
struct _DICacheHashEntry {
    gint valid;
    mSpiderUrl *url;       /* "Key" of this hash entry */
...
{
    return (!a_Url_cmp((mSpiderUrl *)v1, (mSpiderUrl *)v2));
}
...
{
    return g_str_hash(URL_STR(((mSpiderUrl *)key)));
}
...
static DICacheEntry *Dicache_entry_new(void)
{
    ...
    entry->width = 0;
    entry->height = 0;
    entry->type = MSPIDER_IMG_TYPE_NOTSET;
    entry->cmap = NULL;
...
DICacheEntry *a_Dicache_get_entry(const mSpiderUrl *Url)
{
    ...
    for (entry = hash_entry->next; (entry && entry->next);
        entry = entry->next);
 ...
 */
static DICacheEntry *Dicache_get_entry_version(const mSpiderUrl *Url,
                                               gint version)
{
    DICacheHashEntry *hash_entry = g_hash_table_lookup(dicache_hash, Url);
 ...
 */
static void Dicache_remove(const mSpiderUrl *Url, gint version)
{
    DICacheHashEntry *hash_entry = g_hash_table_lookup(dicache_hash, Url);
    DICacheEntry *entry;
    DICacheEntry *prev;
    ...
    entry = hash_entry ? hash_entry->next : NULL;
    prev = entry;
    ...
    if ( Op == CA_Send ) {
        ...
        if ( DicEntry->State == DIC_Write ) {
            for ( i = 0; i < DicEntry->height; ++i )
                ...
                a_Image_write (Image, DicEntry->ImageBuffer + i*DicEntry->width*3, 
                                i, FALSE);
                Image->dw->state = DW_IMAGE_FILLING;
                Image->dw->Y = i;
        }
    }
    else if ( Op == CA_Close || Op == CA_Abort ) {
        Web->Image->dw->state = DW_IMAGE_DONE;
                         ...
                         gint width, gint height, mSpiderImgType type)
{
    ...
    DicEntry->ImageBuffer = malloc(sizeof(guchar) * Size * 4);
                        ...
                        gint num_colors_max, gint bg_index)
{
    ...
    a_Image_set_cmap (Image, DicEntry->cmap, 3 * num_colors);
                        ...
                        gint num_colors_max, gint bg_index, guchar ** rcmap)
{
    ...
    *rcmap = g_new0 (guchar, 3 * num_colors_max);
    memcpy (*rcmap, DicEntry->cmap, 3 * num_colors);
...
#ifdef ENABLE_ANIMATION
#define MAX_DW_LINEBUF  4096
static guchar dw_line_buf[MAX_DW_LINEBUF];

#define valX3(val)(((val)<<1)+(val))

static guchar * DwImage_line (DwImage* dw, const guchar *buf, const guchar *cmap, gint y)
{
    guint x;

    if (cmap) {
        for (x = 0; x < dw->current_frame->width; x++)
            memcpy (dw_line_buf + valX3(x), cmap + valX3(buf[x]), 3);
    }
    else {
        fprintf (stderr, "Gif:: WARNING, image lacks a color map\n");
    }
    return dw_line_buf;
}
#endif
