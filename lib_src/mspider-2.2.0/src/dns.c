 ...
 * Copyright (C) 1999, 2000, 2001 Jorge Arellano Cid <jcid@mspider.org>
 *
...
void a_Dns_init(void)
{
   ...
   DEBUG_MSG(5, "mspider_dns_init: Here we go!\n");
   dns_queue_size = 0;
...
static GSList *Dns_note_hosts(GSList *list, int af, struct hostent *host)
{
   ...
   if (host->h_length > MSPIDER_ADDR_MAX)
      return list;
   for (i = 0; host->h_addr_list[i]; i++) {
      mSpiderHost *dh = g_new0(mSpiderHost, 1);
      dh->af = af;
...
#else
#ifdef _NOUNIX_
      if ((host = resolvGetHostByName(dns_server[channel].hostname,hostbuf,1024)) == NULL)
      {
          printf ("resolvGetHostByName return error\n");
      }
      else
      {
...
static void Dns_server_req(gint channel, const char *hostname)
{
   ...
   g_timeout_add_full (G_PRIORITY_DEFAULT, 200, (GSourceFunc)Dns_timeout_client,
                      GINT_TO_POINTER(dns_server[channel].channel),NULL);
...
static gboolean Dns_timeout_client(gpointer data)
{
   ...
   if (timeout_count <= G_DNS_MAX_TIMEOUT) {
   /* IP not already solved, keep on trying... */
        ...
        /*dns solved fail!*/
        timeout_count = 0;
        page_error (current_browser->dd, ERR_DNSSOLVE);
        return FALSE;
   }
}
