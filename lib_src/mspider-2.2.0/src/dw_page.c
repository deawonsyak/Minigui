...
extern char tabstatck[0x40];
extern int intable;
void a_Dw_page_add_text_ex (DwPage *page, char *text, DwStyle *style);
#endif
static void Dw_page_init          (DwPage *page);
static void Dw_page_class_init    (DwPageClass *klass);

static void Dw_page_finalize      (GObject *object);
...
#define Dw_page_word_wrap(page, word_ind) \
   ((DW_PAGE_GET_CLASS(page))->word_wrap (page, word_ind))
...
GType a_Dw_page_get_type (void)
{
   ...
   if (!type) {
      ...
      type = g_type_register_static (DW_TYPE_CONTAINER, "DwPage", &info, 0);
   }
...
DwWidget* a_Dw_page_new (void)
{
   ...
   object = g_object_new (DW_TYPE_PAGE, NULL);
...
static void Dw_page_init (DwPage *page)
{
   ...
   /* random values */
#ifdef AUTOFOLD_SUPPROT
   page->avail_width = g_rcScr.right-g_rcScr.left;
#else
   page->avail_width = 320;
#endif
...
#ifdef _TOOL_TIP_
   page->hover_tooltip = NULL; /* The tooltip under the button. No ref hold. */
#endif

   page->num_iframe = 0;
   page->is_menupage = FALSE;
...
static void Dw_page_class_init (DwPageClass *klass)
{
   ...
   object_class->finalize = Dw_page_finalize;
...
/*
 * Destroy the page (standard GObject function)
 */
static void Dw_page_finalize (GObject *object)
{
   ...
   for (i = 0; i < page->num_words; i++) {
      word = &page->words[i];
      if (word->content.data.widget && word->content.type == DW_CONTENT_WIDGET)
      {
          g_object_unref (G_OBJECT(word->content.data.widget));
      }
         ...
         * and related). */
         p_Dw_viewport_remove_anchor (widget, word->content.data.anchor);
      }
   ...
   /* Make sure we don't own widgets anymore. Neccessary before call of
                                   ...
                                   DwAllocation *allocation)
{
   ...
   for (line_index = 0; line_index < page->num_lines; line_index++) {
      ...
      y_cursor = Dw_page_line_total_y_offset (page, line);
...
static void Dw_page_real_word_wrap (DwPage *page, gint word_ind)
{
   ...
   avail_width = page->avail_width - p_Dw_style_box_diff_width (widget->style) - page->inner_padding;
                                      ...
                                      DwRequisition *size)
{
   ...
   if (DW_WIDGET_USES_HINTS (widget)) {
	  ...
	  if (DW_IS_IMAGE(widget)){
          if(widget->style->width == DW_STYLE_LENGTH_AUTO &&
                  DW_STYLE_IS_PER_LENGTH (widget->style->height)){
            wscale = (double)((requisition.ascent + requisition.descent)/
				(DW_STYLE_PER_LENGTH_VAL(widget->style->height) * (avail_ascent + avail_descent)));
			requisition.width = (int)((requisition.width)/wscale);
          }

          if(widget->style->height == DW_STYLE_LENGTH_AUTO &&
                  DW_STYLE_IS_PER_LENGTH (widget->style->width)){
			  hscale = (double) ((requisition.width)/
					  (DW_STYLE_PER_LENGTH_VAL (widget->style->width) * avail_width));
              requisition.ascent = (int)((requisition.ascent)/hscale);
              requisition.descent = (int)((requisition.descent)/hscale);
          }
      }
 ...
 */
void Dw_page_rewrap (DwPage *page)
{
   DwWidget *widget;
   gint i, word_index;
   DwPageWord *word;
   ...
   for (; word_index < page->num_words; word_index++) {
      ...
      DEBUG_MSG(DEBUG_REWRAP_LEVEL,
                ...
                "in page with %d word(s) Dw_page_rewrap\n",
                               ...
                               DwRectangle *area)
{
   ...
   /*
    * the following one line is to eliminate the iframe display error, 
    * some time it is display correct, some time it is display only a 
    * part of line
   (&((DwWidget*)(page))->allocation)->ascent =(page)->lines[0].ascent + 5; 
    */
	   printf("func:%s line:%d\n", __func__, __LINE__);
   ...
   y_viewport =
      p_Dw_widget_y_world_to_viewport (widget, widget->allocation.y + y_widget);
   y_viewport_base = y_viewport + line->ascent;
   printf("func:%s line:%d\n", __func__, __LINE__);

   for (word_index = line->first_word; word_index < line->last_word;
        word_index++) {
	  printf("func:%s line:%d word_index:%d\n", __func__, __LINE__, word_index);
	  printf("page->words:%x\n", page->words);
      word = &page->words[word_index];
	   printf("func:%s line:%d hdc:%x, font:%x, HDC_SCREEN:%x\n", __func__, __LINE__,hdc, word->style->font->font, HDC_SCREEN);
      diff = 0;
		SelectFont (HDC_SCREEN, word->style->font->font);
		 printf("func:%s line:%d\n", __func__, __LINE__);
	  
      SelectFont (hdc, word->style->font->font);
	   printf("func:%s line:%d\n", __func__, __LINE__);
      SetBkMode (hdc, BM_TRANSPARENT);
	   printf("func:%s line:%d\n", __func__, __LINE__);
      SetTextColor (hdc, word->style->color->pixel);
	   printf("func:%s line:%d\n", __func__, __LINE__);
      SetPenColor (hdc, word->style->color->pixel);
	  printf("func:%s line:%d\n", __func__, __LINE__);
      ...
      case DW_CONTENT_TEXT:
         ...
         if (word->style->background_color &&
             word->size.width > 0)
            p_Dw_widget_draw_box (widget, hdc, word->style, area,
                                  x_widget,
         ...
         if (word->space_style->background_color &&
             word->eff_space > 0)
            p_Dw_widget_draw_box (widget, hdc, word->space_style, area,
         ...
         if (word_bg_color) {
            SetBrushColor (hdc, word_bg_color->pixel);
            SetBkColor (hdc, word_bg_color->pixel);
            
         }
         TextOut (hdc, x_viewport, 
                         y_viewport_base + diff - word->size.ascent, 
         ...
         if (word->style->text_decoration & DW_STYLE_TEXT_DECORATION_UNDERLINE)
        {
            ...
            MoveTo (hdc, x_viewport, y_viewport_base + 1 + diff);
            LineTo (hdc, x_viewport + word->size.width - 1,
        ...
        {
            MoveTo (hdc, x_viewport + word->size.width,
                           y_viewport_base + 1 + diff);
            LineTo (hdc, x_viewport + word->size.width + word->eff_space - 1,
                           y_viewport_base + 1 + diff);
        }
		 printf("func:%s line:%d\n", __func__, __LINE__);
         ...
         for (layer = 0; layer < DW_HIGHLIGHT_NUM_LAYERS; layer++) {
            if (word->hl_start[layer] != -1) {
               ...
               GetTextExtent (hdc, word->content.data.text, 
                              word->hl_start[layer], &ext);
               start_hl = x_viewport + ext.cx;
                
               GetTextExtent (hdc,  
                              word->content.data.text + word->hl_start[layer],
                              eff_hl_end - word->hl_start[layer], &ext);
                width_hl = ext.cx;
               ...
               if (width_hl != 0) {
                  /* Draw background for highlighted text. */
                    if (word_bg_color) {
                        SetBrushColor (hdc, word_bg_color->inverse_pixel);
						printf("####inverse_pixel = 0x%x\n", word_bg_color->inverse_pixel);
                        FillBox (hdc, start_hl, y_viewport_base - word->size.ascent,
                                    width_hl, 
                                    word->size.ascent + word->size.descent);

                        SetBkColor (hdc, word_bg_color->inverse_pixel);
                    }
                    SetTextColor (hdc, word->style->color->inverse_pixel);
                    TextOutLen (hdc, start_hl, 
                                    y_viewport_base + diff - word->size.ascent, 
                                word->content.data.text + word->hl_start[layer],
                                eff_hl_end - word->hl_start[layer]);

                    SetPenColor (hdc, word->style->color->inverse_pixel);
                    if (word->style->text_decoration
                            & DW_STYLE_TEXT_DECORATION_UNDERLINE) {
                        MoveTo (hdc, start_hl, y_viewport_base + 1 + diff);
                        LineTo (hdc, start_hl + width_hl - 1, 
                                     y_viewport_base + 1 + diff);
                    }
                    if (word->style->text_decoration
                            & DW_STYLE_TEXT_DECORATION_LINE_THROUGH) {
      ...
      case DW_CONTENT_WIDGET:
		...
		printf("func:%s line:%d\n", __func__, __LINE__);
         ...
         if (p_Dw_widget_intersect (child, area, &child_area))
            a_Dw_widget_draw (child, hdc, &child_area);
                          ...
                          DwRectangle *area)
{
   ...
   p_Dw_widget_draw_widget_box (widget, hdc, area, FALSE);
	...
	hdc = HDC_SCREEN;
    dd = (mSpiderDoc*) GetWindowAdditionalData2 (viewport->hwnd);
#ifdef ENABLE_LINKTRAVE
    bw = dd->bw;
#endif
   ...
   for (; line_index < page->num_lines; line_index++) {
      ...
      Dw_page_draw_line (page, hdc, line, area);
   }
#ifdef JS_SUPPORT
   ...
   jsinterpreter = jsobj->jsinterpreter;
	if ( !jsinterpreter ) {
		return;
	}
   if ( jsobj->onload && jsobj->is_onload) {
        jsobj->is_onload = 0;
		jscode.source = jsobj->onload;
		jscode.length = strlen(jsobj->onload);
...
#ifdef ENABLE_LINKTRAVE
    if ((((LinktraveManage*)bw->linktrave_manage)->linktrave_state->mode == LSM_MULIT) &&
        (((LinktraveManage*)bw->linktrave_manage)->linktrave_state->widget == NULL) &&
        ((LinktraveManage*)bw->linktrave_manage)->linktrave_state->ddary)
    {
        dd = ((LinktraveManage*)bw->linktrave_manage)->linktrave_state->ddary[0]; 
        if (dd) {
             a_Linktrave_state_set_widget (((LinktraveManage*)bw->linktrave_manage)->linktrave_state,
                                                                 ((DwViewport*)(dd->viewport))->child);
            ((LinktraveManage*)bw->linktrave_manage)->linktrave_state->link_block = dd->html_block; 
            ((LinktraveManage*)bw->linktrave_manage)->link_number_focus = -2;
            SendMessage (dd->docwin, MSG_SETFOCUS, 0, 0);   
        }
    }

    if (((LinktraveManage*)bw->linktrave_manage)->link_number_focus == -2) {
        ((LinktraveManage*)bw->linktrave_manage)->link_number_focus = 0;
        a_Linktrave_step (((LinktraveManage*)bw->linktrave_manage)->linktrave_state,
                                     ((LinktraveManage*)bw->linktrave_manage)->link_number_focus, LTS_FORWARD);
    }
 ...
 */
gint Dw_page_find_word (DwPage *page, gint x, gint y)
{
...
static gint Dw_page_find_link (DwPage *page, gint x, gint y)
{
   ...
   if ((word_index = Dw_page_find_word (page, x, y)) == -1)
      return -1;
   else
      return page->words[word_index].style->x_link;
}
            ...
            if (x >= word_start_x && x < next_word_start_x) {
               ...
               if (word->content.type == DW_CONTENT_TEXT) {
                  ...
                  while ((next_word_x = word_start_x +
                         ...
                         <= x) {
                     char_pos++;
                     if ((size_t)char_pos >= strlen(word->content.data.text)) break;
				  }
                                       ...
                                       DWORD flags)
{
   ...
   return Dw_page_send_selection_event (DW_PAGE (widget),
    ...
    if (page->hover_link != -1 && HIWORD (flags) == MSG_RBUTTONDOWN) {
        g_signal_emit (G_OBJECT (widget), page_signals[LINK_CLICKED], 0,
                                     page->hover_link, x, y, flags,
                                     &dummy);
        return TRUE;
    }
    else
        page->pressed_link = page->hover_link;
                                        ...
                                        DWORD flags)
{
   ...
   return Dw_page_send_selection_event (DW_PAGE (widget),
    ...
    if (page->hover_link != -1 && page->hover_link == page->pressed_link) {
        g_signal_emit (G_OBJECT (widget), page_signals[LINK_CLICKED], 0,
                                     page->hover_link, x, y, flags,
                                     &dummy);
        page->pressed_link = -1;
                                     ...
                                     DwStyle *style)
{
   ...
   SetRect (&word->rc_drawn, 0, 0, 0, 0);
   word->orig_space = 0;
   word->eff_space = 0;

#ifdef ENABLE_LINKTRAVE
    if (style->x_link != -1)
        word->content.x_link = style->x_link;
    else
        word->content.x_link = -1;
#endif /* ENABLE_LINKTRAVE */
                                     ...
                                     DwRequisition *size)
{
   SIZE ext;
   FONTMETRICS fm;
   GetFontMetrics (style->font->font, &fm);
   
   SelectFont (HDC_SCREEN, style->font->font);
   GetTextExtent (HDC_SCREEN, text, -1, &ext);
   size->width = ext.cx;
   size->ascent = fm.ascent + 2;
   size->descent = fm.descent + 2;
...
void a_Dw_page_add_text(DwPage *page, char *text, DwStyle *style)
{
	...
	char* my_text = text;
	char* new_word = NULL;
	DwPageWord *word = NULL;

	my_text_len = strlen(text);
    Dw_page_calc_text_size (page, text, style, &size);

	if ( intable && tabstatck[intable-1] ) {
		do {
			if ( GetFirstMCharLen (style->font->font, my_text, my_text_len) == 1 ) {
				word_len = GetFirstWord (style->font->font, my_text, my_text_len, &wi);
                if (word_len > 0) {
					new_word = g_malloc ((size_t)(word_len + 1));
					memcpy (new_word, my_text, word_len);
					new_word [word_len] = '\0';
					my_text += word_len;
					my_text_len -= word_len;
					GetTextExtent (HDC_SCREEN, new_word, -1, &ext);
					new_word_width = ext.cx;
                } else {
					MSG ("!!!add_a_word failed (%s)\n", my_text);
					new_word = NULL;
					break;
				}
            } else {
            ...
            word = Dw_page_add_word (page, new_word_width, 
                            size.ascent, size.descent, style);
            word->content.type = DW_CONTENT_TEXT;
            word->content.data.text = new_word;
            Dw_page_word_wrap (page, page->num_words - 1);
			/* note! */
			a_Dw_page_add_linebreak(page, style);
        } while (my_text_len > 0);

        g_free (text);
	} else {
		a_Dw_page_add_text_ex (page, text, style);
	}
}
#endif
...
#endif
{
    DwPageWord *word, *prev_word = NULL;
    DwRequisition size;
    gint32 avail_width, last_line_width = 0, prev_orig_space = 0;
    DwWidget *widget = DW_WIDGET (page);
#ifdef AUTOFOLD_SUPPORT
	gint32 last_line_width2 = 0;
	DwRequisition size2;
#endif
    ...
    if (page->num_words > 0) {
        prev_word = &page->words[page->num_words - 1];
        if (prev_word->content.type == DW_CONTENT_TEXT) {
            last_line_width = page->last_line_width;
            prev_orig_space = prev_word->orig_space;
        }
    }

    last_line_width += prev_orig_space;

    if (last_line_width + size.width > avail_width ) {
        char* new_word;
        int new_word_width;
        char* my_text = text;
        int my_text_len = strlen (text);

        SelectFont (HDC_SCREEN, style->font->font);
        do {
#if 0
            if (GetFirstMCharLen (style->font->font, 
                                    my_text, my_text_len) == 1) {
#else
            if (1) {
#endif
                WORDINFO wi;
                ...
                word_len = GetFirstWord (style->font->font, my_text, 
                                my_text_len, &wi);
...
#ifdef AUTOFOLD_SUPPORT
			Dw_page_calc_text_size (page, new_word, style, &size2);
			last_line_width2 = page->last_line_width;
			if ( last_line_width2 + size2.width >= avail_width ) {
				a_Dw_page_add_linebreak(page, style);
			}
#endif
            word = Dw_page_add_word (page, new_word_width, 
 ...
 * the caller.
 */
gboolean a_Dw_page_add_anchor (DwPage *page, const char *name, DwStyle *style)
{
   DwPageWord *word;
   char *copy;
   gint32 y;
    ...
    */
   if ((copy = p_Dw_viewport_add_anchor (DW_WIDGET(page), name, y)) == NULL)
      return FALSE;
   else {
...
void a_Dw_page_add_linebreak (DwPage *page, DwStyle *style)
{
   ...
   if (page->num_words == 0 ||
        ...
        * (why would someone else place it here?), ...  */
        FONTMETRICS fm;
        GetFontMetrics (style->font->font, &fm);
      word = Dw_page_add_word (page, 0, fm.ascent, fm.descent, style);
    }
   else
   ...
   for (i = 0; i < page->num_lines; i++) {
      ...
      for (j = page->lines[i].first_word; j < page->lines[i].last_word; j++)
         if (page->words[j].style->x_link == link) {
            ...
            style_attrs.color =
               a_Dw_style_color_new (new_color, widget->viewport->hwnd);
            page->words[j].style =
               a_Dw_style_new (&style_attrs, widget->viewport->hwnd);
            /* unref'ing it before may crash mspider! */
                                             ...
                                             DwAllocation *allocation)
{
   ...
   if (start == 0)
      ...
      if (word->content.type == DW_CONTENT_TEXT) {
         SIZE ext;
         SelectFont (HDC_SCREEN, word->style->font->font);
         GetTextExtent (HDC_SCREEN, word->content.data.text, start, &ext);
   ...
   if (word->content.type == DW_CONTENT_TEXT) {
      ...
      if (end == l)
         ...
         SIZE ext;
         SelectFont (HDC_SCREEN, word->style->font->font);
         GetTextExtent (HDC_SCREEN, word->content.data.text, end, &ext);
         allocation->width = ext.cx;
      }
   } else {
      if (end == 1)
         allocation->width = word->size.width;
