...
static void Dw_table_finalize      (GObject *object);
...
GType a_Dw_table_get_type (void)
{
   ...
   if (!type) {
      ...
      type = g_type_register_static (DW_TYPE_CONTAINER, "DwTable", &info, 0);
   }
...
DwWidget* a_Dw_table_new (void)
{
   ...
   object = g_object_new (DW_TYPE_TABLE, NULL);
...
static void Dw_table_init (DwTable *table)
{
   ...
   table->num_cell_max= 1; /* 8 */
   table->num_cell = 0;
   table->cell_height = NULL;
...
static void Dw_table_class_init (DwTableClass *klass)
{
   ...
   object_class->finalize = Dw_table_finalize;
...
static void Dw_table_finalize (GObject *object)
{
   ...
   for (i = 0; i < table->num_children; i++)
      if (table->children[i]) {
         if (table->children[i]->type == DW_TABLE_CELL)
            g_object_unref(G_OBJECT(table->children[i]->data.cell.widget));
   ...
   table->num_children = 0;
	...
	if ( table->cell_height ) {
		g_free(table->cell_height);
	}
                                   ...
                                   DwRequisition *requisition)
{
   ...
   gint32  avail_height, avg_height , num_rows , av_child_height;
   gboolean focus_height;
   gint32 focus_value;
   ...
   for (col = 0; col < table->num_cols; col++)
    ...
    if (table->num_rows > 0) {
      ...
      if (DW_STYLE_IS_ABS_LENGTH (widget->style->height))
         ...
         avail_height = table->avail_ascent
            * MIN (DW_STYLE_PER_LENGTH_VAL (widget->style->height), 1.0);

      avg_height = avail_height / table->num_rows;
   ...
   for (row = 0; row < table->num_rows; row++) {
      ...
      for (col = 0; col < table->num_cols; col++) {
         ...
         if (CHILD_DEFINED (table, n)) {
            ...
            if (child->style->valign == DW_STYLE_VALIGN_BASELINE)
              ...
              if (child->style->height != DW_STYLE_LENGTH_AUTO) {
                  if (DW_STYLE_IS_ABS_LENGTH (child->style->height))
                     av_child_height = DW_STYLE_ABS_LENGTH_VAL (child->style->height);
                  else if (DW_STYLE_IS_PER_LENGTH (child->style->height))
                     av_child_height = table->avail_ascent
                        * MIN (DW_STYLE_PER_LENGTH_VAL (child->style->height), 1.0);

                  if (child_height < av_child_height) 
                    child_height = av_child_height;

                  if (child_height > focus_value)
                    focus_value = child_height;
                
                    focus_height = TRUE; 
              }

            
            if ((child_height < avg_height) && (child->style->height == DW_STYLE_LENGTH_AUTO)) {

                child_height = avg_height;
            }
			...
			if ( widget->style->border_width.top == 0 ) {
				if ( row == 0 && col == 0 ) {
					idx = 0;
				}

				if ( idx ==  table->num_cell ) {
					table->num_cell++;
					a_List_resize(table->cell_height, table->num_cell,
						table->num_cell_max);
				}

				/*if ( idx-1 >= 0 ) {
					table->cell_height[idx] = 
						table->cell_height[idx-1] + child_height;
				} else*/ {
					table->cell_height[idx] = child_height;
				}
      ...
      if (focus_height) {
        table->cum_height[row+1] = table->cum_height[row] + focus_value
                                        + widget->style->border_spacing;
      }

        avail_height -= table->cum_height[row+1];

        if ((num_rows -1) > 0)
            avg_height = avail_height / (num_rows -1);
        else
            avg_height = 0;
      ...
      for (col = 0; col < table->num_cols; col++) {
         ...
         if (CHILD_DEFINED (table, n)) {
			...
			if ( widget->style->border_width.top ) {
				...
				cell_height = table->cell_height[idx++];
				child_allocation.y = cury;
				cury += cell_height;
				cury += widget->style->border_spacing;
			}
#else
            cell_height =
               table->cum_height[row + table->children[n]->data.cell.rowspan] -
                           ...
                           DwRectangle *area)
{
   ...
   width = DW_WIDGET_CONTENT_WIDTH(widget);
	...
	if ( widget->style->border_width.top != 0 ) {
   		for (i = 0; i < table->num_rows; i++) {
      		if (table->row_style[i]) {
         		p_Dw_widget_draw_box (widget, hdc, table->row_style[i], area,
...
#if 0
            Rectangle (hdc, 
                p_Dw_widget_x_world_to_viewport (child, child->allocation.x) - 1,
                p_Dw_widget_y_world_to_viewport (child, child->allocation.y) - 1,
                p_Dw_widget_x_world_to_viewport (child, child->allocation.x) + child->allocation.width + 1,
                p_Dw_widget_y_world_to_viewport (child, child->allocation.y) + DW_WIDGET_HEIGHT(child) + 1);
#endif
   ...
   Rectangle (hdc, 
        ...
        p_Dw_widget_x_world_to_viewport (widget, widget->allocation.x) + widget->allocation.width,
        p_Dw_widget_y_world_to_viewport (widget, widget->allocation.y) + DW_WIDGET_HEIGHT(widget));
#endif
}
                                               ...
                                               GdkEventButton *event)
{
   ...
   if (row == table->num_rows)
      ...
      if (y < table->cum_height[row] + offy) {
          ...
          * is never the case in mspider. But it works.) */
                                         ...
                                         gint row)
{
   ...
   if (sub->table->children[n]) {
      ...
      return (start_col <= sub->start_col &&
              start_col + colspan >= sub->end_col);	/* why?! */
   } else
      return FALSE;
}
