...
GType a_Dw_table_cell_get_type (void)
{
   ...
   if (!type) {
      static const GTypeInfo info = {
         ...
         (GClassInitFunc) Dw_table_cell_class_init,
         (GClassFinalizeFunc) NULL,
         (gconstpointer) NULL,
         sizeof (DwTableCell),
         0,
         (GInstanceInitFunc) Dw_table_cell_init
      };

      type = g_type_register_static (DW_TYPE_ALIGNED_PAGE, "DwTableCell", &info, 0);
   }
...
/*
 * Standard GObject function: Create a new table_cell item.
...
DwWidget* a_Dw_table_cell_new (DwTableCell *ref_cell)
{
   ...
   object = g_object_new (DW_TYPE_TABLE_CELL, NULL);
...
static gint32 Dw_table_cell_get_value (DwAlignedPage *aligned_page)
{
   ...
   if (cell->char_word_index == -1) {
      ...
      GetTextExtent (HDC_SCREEN, word->content.data.text, cell->char_word_pos, &ext);
      w += ext.cx;
   }
