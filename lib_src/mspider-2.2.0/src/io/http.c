 ...
 * Copyright (C) 2000, 2001 Jorge Arellano Cid <jcid@mspider.org>
 *
...
static int nr_curr_sockets = 0;	/* the number of current active sockets */

int a_Http_get_nr_sockets (void)
{
    return nr_curr_sockets;
}

mSpiderUrl *HTTP_Proxy = NULL;
gchar *HTTP_Proxy_Auth_base64 = NULL;
 ...
 * in mspiderrc, but as this constitutes a security problem, it was disabled.
 *
...
gchar *a_Http_make_query_str(const mSpiderUrl *url, gboolean use_proxy)
{
   ...
   if (URL_PORT(url) && URL_PORT(url) != MSPIDER_URL_HTTP_PORT)
   ...
   if (a_Auth_byurl((mSpiderUrl *) url)) {
      g_string_sprintfa(auth, "Authorization: %s\r\n",
         a_Auth_byurl((mSpiderUrl *) url)->str);
   }
...
static int Http_connect_socket(ChainLink *Info)
{
   ...
   case AF_INET:
   {
      ...
      sin->sin_port = S->port ? htons(S->port) : htons(MSPIDER_URL_HTTP_PORT);
   ...
   case AF_INET6:
   {
      ...
      sin6->sin6_port = S->port ? htons(S->port) : htons(MSPIDER_URL_HTTP_PORT);
   ...
   if (!S->GioCh) {
      DEBUG_MSG (1, "g_io_channel_unix_new called with fd: %d\n", S->SockFD);
   ...
   DEBUG_MSG (1, "g_io_add_watch called with return value: %d\n", watch_id);
   status = connect(S->SockFD, (struct sockaddr *)&name, socket_len);
   if ( status == -1 && errno != EINPROGRESS ) {
      DEBUG_MSG(1, "Http_connect_socket ERROR: %s\n", g_strerror(errno));
      g_io_channel_close(S->GioCh);
...
static void Http_send_query(ChainLink *Info, SocketData_t *S)
{
   ...
   /* send query */
   DEBUG_MSG(1, "Sending query to %s (fd: %d)...\n", URL_HOST_(S->Url), S->SockFD);
 ...
 */
static gint Http_must_use_proxy(const mSpiderUrl *url)
{
   gint i;
...
static gint Http_get(ChainLink *Info, void *Data1, void *Data2)
{
   ...
   DEBUG_MSG(1, "DNS solving %s", URL_HOST_(S->Url));
        ...
        /* HTTP query branch */
         ...
         case OpStart:
            ...
            if (Http_get(Info, Data1, Data2) < 0) {
               ...
               DEBUG_MSG(1, "ERROR: %s", g_strerror(S->Err));
               Http_socket_free(SKey);
               a_Chain_fcb(OpAbort, Info, NULL, NULL);
            }
            break;
      ...
      if (Dir == FWD) {
         ...
         case OpEnd:
            ...
            if (S) {
               ...
               DEBUG_MSG (1, "Query sent, waiting for reply...");
               Http_socket_free(SKey);
            }
            break;
         case OpAbort:
