...
/*
 * mSpider's signal driven IO engine
 */
...
void a_IO_write_chunk(gint FD, void *Key, void *Buf, size_t BufSize)
{
   ...
   printf("_IO_write_chunk is called !\n");
...
#ifdef _NOUNIX_
   on = 1;
   if (ioctl(r_io->FD,FIONBIO,(int)&on) == -1)
   	{
   	    printf ("set non-blocking failure. \n");
   	}
#else
