...
/*
 * The jpeg decoder for mspider. It is responsible for decoding JPEG data
...
typedef enum {
   MSPIDER_JPEG_INIT,
   MSPIDER_JPEG_STARTING,
   MSPIDER_JPEG_READING,
   MSPIDER_JPEG_DONE,
   MSPIDER_JPEG_ERROR
} mSpiderJpegState;
...
typedef struct {
   ...
   struct mSpiderJpeg *jpeg;      /* a pointer back to the jpeg object */
} my_source_mgr;
 ...
 */
static mSpiderJpeg *Jpeg_new(mSpiderImage *Image, mSpiderUrl *url, gint version);
static void Jpeg_callback(int Op, CacheClient_t *Client);
static void Jpeg_write(mSpiderJpeg *jpeg, void *Buf, guint BufSize);
static void Jpeg_close(mSpiderJpeg *jpeg, CacheClient_t *Client);
...
static void Jpeg_close(mSpiderJpeg *jpeg, CacheClient_t *Client)
{
   ...
   if (jpeg->state != MSPIDER_JPEG_DONE) {
...
{
   mSpiderJpeg *jpeg = ((my_source_mgr *) cinfo->src)->jpeg;
...
{
   my_source_mgr *src;
   mSpiderJpeg *jpeg = g_malloc(sizeof(mSpiderJpeg));

   jpeg->Image = Image;
   jpeg->url = url;
   jpeg->version = version;

   jpeg->state = MSPIDER_JPEG_INIT;
   jpeg->Start_Ofs = 0;
   jpeg->Skip = 0;
...
static void Jpeg_write(mSpiderJpeg *jpeg, void *Buf, guint BufSize)
{
   ...
   if (setjmp(jpeg->jerr.setjmp_buffer)) {
      ...
      jpeg->state = MSPIDER_JPEG_ERROR;
   }

   /* Process the bytes in the input buffer. */
   if (jpeg->state == MSPIDER_JPEG_INIT) {
      ...
      if (jpeg_read_header(&(jpeg->cinfo), TRUE) != JPEG_SUSPENDED) {
         ...
         jpeg->state = MSPIDER_JPEG_STARTING;
      }
   }
   if (jpeg->state == MSPIDER_JPEG_STARTING) {
      /* decompression step 5 (see libjpeg.doc) */
      if (jpeg_start_decompress(&(jpeg->cinfo))) {
         jpeg->y = 0;
         jpeg->state = MSPIDER_JPEG_READING;
      }
   }
   if (jpeg->state == MSPIDER_JPEG_READING) {
      ...
      if (jpeg->y == jpeg->cinfo.image_height) {
         ...
         jpeg_destroy_decompress(&(jpeg->cinfo));
         jpeg->state = MSPIDER_JPEG_DONE;
      }
      g_free(linebuf);
   }
   if (jpeg->state == MSPIDER_JPEG_ERROR) {
      jpeg_destroy_decompress(&(jpeg->cinfo));
      jpeg->state = MSPIDER_JPEG_DONE;
   }
}
