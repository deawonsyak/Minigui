 ...
 * pass in .mspider/bookmarks.html and it will return
 * /home/imain/.mspider/bookmarks.html
 *
...
gchar *a_Misc_prepend_user_home(const char *file)
{
   ...
   char* tmp = (char*)g_malloc(64);
   sprintf(tmp,"%s%s", "/userdata/out/",file);
   
   //static char tmp[64]= "/userdata/out/.mspider/cookiesrc";
   return tmp;
}
 ...
 * only checks the few mime types that mSpider supports.
 *
