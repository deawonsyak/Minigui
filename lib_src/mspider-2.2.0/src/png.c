 ...
 * License: GPL
 *
 * The png decoder for mSpider. It is responsible for decoding PNG data
...
typedef
struct _mSpiderPng {
   mSpiderImage *Image;           /* Image meta data */
   mSpiderUrl *url;               /* Primary Key for the dicache */
...
Png_datainfo_callback(png_structp png_ptr, png_infop info_ptr)
{
   ...
   a_Dicache_set_parms(png->url, png->version, png->Image,
                       (guint)png->width, (guint)png->height,
 ...
 *
 * Client->CbData  : pointer to previously allocated mSpiderPng work area.
...
static void Png_callback(int Op, CacheClient_t *Client)
{
   ...
   if ( Op ) {
      ...
      if (setjmp(png->jmpbuf))
         g_warning("PNG: can't destroy read structure\n");
      else if (png->png_ptr)
 ...
 */
static mSpiderPng *Png_new(mSpiderImage *Image, mSpiderUrl *url, gint version)
{
   mSpiderPng *png = g_new0(mSpiderPng, 1);
 ...
 * Call: mSpider calls this with more data/eod
 * Data: raw image data
 */
