...
/*
 * Read tokens from mspiderrc and set values in the prefs structure.
...
static guint Prefs_parser(GScanner *scanner)
{
   ...
   case DRC_TOKEN_USER_AGENT:
   {
      ...
      if((option = strstr(prefs_value, "%v"))) {
         option[1] = 's';
         prefs.user_agent = g_strdup_printf(prefs_value,EMSPIDER_VERSION);
   ...
   case DRC_TOKEN_FONT_FACTOR:
      prefs.font_factor = (double)strtol(scanner->value.v_string, NULL, 10);
      break;
...
static gint Prefs_load(void)
{
   ...
   /* Here we load and set options from mspiderrc */
   file = a_Misc_prepend_user_home(".mspider/mspiderrc");
#ifdef _NOUNIX_
   fd = open(file, O_RDONLY, 0440);
#else
   fd = open(file, O_RDONLY);
#endif
   g_free(file);
#ifdef _NOUNIX_
   if (fd < 0 && (fd = open(MSPIDERRC_SYS, O_RDONLY,0440)) < 0)
#else
   if (fd < 0 && (fd = open(MSPIDERRC_SYS, O_RDONLY)) < 0)
#endif
...
void a_Prefs_init(void)
{
    ...
    prefs.start_page = a_Url_new(MSPIDER_START_PAGE, NULL, 0, 0, 0);
    prefs.home = a_Url_new(MSPIDER_HOME, NULL, 0, 0, 0);
    ...
    if(set_font_factor > 0)
        prefs.font_factor = set_font_factor;
    else
