 ...
 * Copyright (C) 2001 Jorge Arellano Cid <jcid@mspider.org>
...
/*
 * Parse and normalize all URL's inside mSpider.
 ...
 */
gchar *a_Url_str(const mSpiderUrl *u)
{
   /* Internal url handling IS transparent to the caller */
   mSpiderUrl *url = (mSpiderUrl *) u;
 ...
 */
const gchar *a_Url_hostname(const mSpiderUrl *u)
{
   gchar *p;
   /* Internal url handling IS transparent to the caller */
   mSpiderUrl *url = (mSpiderUrl *) u;
...
/*
 *  Create a mSpiderUrl object and initialize it.
 *  (buffer, scheme, authority, path, query and fragment).
 */
static mSpiderUrl *Url_object_new(const gchar *uri_str)
{
   mSpiderUrl *url;
   gchar *s, *p, *x;
   GString *tmpstr;
 ...
 *  Free a mSpiderUrl
 */
void a_Url_free(mSpiderUrl *url)
{
    if (url) {
    if (url->url_string)
...
static GString *Url_resolve_relative(const gchar *RelStr,
                                     mSpiderUrl **BaseUrlPar,
                                     const gchar *BaseStr)
{
   gchar *p, *s, *e;
   gint i;
   GString *SolvedUrl, *Path;
   mSpiderUrl *RelUrl, *BaseUrl = NULL;
 ...
 */
mSpiderUrl* a_Url_new(const gchar *url_str, const gchar *base_url,
                    gint flags, gint32 posx, gint32 posy)
{
   mSpiderUrl *url, *BaseUrl = NULL;
   gchar *urlstring, *p, *s;
   GString *SolvedUrl;
   gint n_ic;
 ...
 */
mSpiderUrl* a_Url_dup(const mSpiderUrl *ori)
{
   mSpiderUrl *url;
 ...
 */
gint a_Url_cmp(const mSpiderUrl *A, const mSpiderUrl *B)
{
   if (!A || !B)
      return 1;
...
/*
 * Set mSpiderUrl flags
 */
void a_Url_set_flags(mSpiderUrl *u, gint flags)
{
   if (u)
      u->flags = flags;
}
...
/*
 * Set mSpiderUrl data (like POST info, etc.)
 */
void a_Url_set_data(mSpiderUrl *u, gchar *data)
{
   if (u) {
      g_free((gchar *)u->data);
...
/*
 * Set mSpiderUrl alt (alternate text to the URL. Used by image maps)
 */
void a_Url_set_alt(mSpiderUrl *u, const gchar *alt)
{
   if (u) {
      g_free((gchar *)u->alt);
      u->alt = g_strdup(alt);
...
/*
 * Set mSpiderUrl target (used to target link at specific frame or window)
 */
void a_Url_set_target(mSpiderUrl *u, const gchar *target)
{
   if (u) {
      if (u->target)
...
/*
 * Set mSpiderUrl scrolling position
 */
void a_Url_set_pos(mSpiderUrl *u, gint32 posx, gint32 posy)
{
   if (u) {
...
/*
 * Set mSpiderUrl referer URL
 */
void a_Url_set_referer(mSpiderUrl *u, mSpiderUrl *ref)
{
   if (u && ref) {
      if (u->referer)
...
/*
 * Set mSpiderUrl ismap coordinates
 * (this is optimized for not hogging the CPU)
 */
void a_Url_set_ismap_coords(mSpiderUrl *u, gchar *coord_str)
{
 ...
 */
gchar *a_Url_decode_hex_str(const char *str)
{
   gchar *new_str, *dest;
   int i, val;
...
gboolean a_Url_does_visited (mSpiderUrl *u)
{
    char *buf;
    int buf_size;
    return a_Capi_get_buf(u, &buf, &buf_size);
}
 ...
 */
gchar *a_Url_parse_hex_path(const mSpiderUrl *u)
{
   const gchar *src;
   gchar *new_uri, *dest;
   int i, val;
