...
typedef struct _mSpiderUrl mSpiderUrl;
...
void a_Url_free(mSpiderUrl *u);
gchar *a_Url_str(const mSpiderUrl *url);
const gchar *a_Url_hostname(const mSpiderUrl *u);
mSpiderUrl* a_Url_dup(const mSpiderUrl *u);
...
char *a_Url_parse_hex_path(const mSpiderUrl *u);
